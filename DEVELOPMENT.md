...
|---------|----------|---------|----------------|
| `npm run dev` | `mobile/.env.local` | `http://localhost:9000` | Your machine (local) |
| `npm run dev:tunnel` | `mobile/.env.tunnel` | `https://xxxxx.ngrok-free.dev` | Public internet |

**How it works:**
- `dotenv -e mobile/.env.local --` in the npm script loads the local config
- `dotenv -e mobile/.env.tunnel --` in the npm script loads the tunnel config
- Each npm command automatically uses the correct environment file
- When you switch between `npm run dev` and `npm run dev:tunnel`, just run the appropriate command and restart Expo (press `r`)
...
This automatically:
1. Sets `EXPO_PUBLIC_API_URL=http://localhost:9000/.netlify/functions` (local)
...
### `mobile/.env.local` (Local Development)
Used by `npm run dev`. Committed to git with a static local URL:

```
EXPO_PUBLIC_API_URL=http://localhost:9000/.netlify/functions
```

**Don't modify** unless you're changing the local dev port.

### `mobile/.env.tunnel` (Remote Development)
Generated by the tunnel script when you run `npm run dev:tunnel`. **Not committed** (in .gitignore):

```
EXPO_PUBLIC_API_URL=https://xxxxx.ngrok-free.dev/.netlify/functions
```
...
Just run the appropriate npm command - dotenv automatically loads the right file:

```bash
npm run dev          # Loads mobile/.env.local (localhost)
npm run dev:tunnel   # Loads mobile/.env.tunnel (ngrok URL)
```
