...
async function uploadToCloudinary(imageUrl, playerName) {
    try {
        console.log(`  → Uploading to Cloudinary...`);
        // Download the image from showdownbot
        const imageResponse = await fetchWithTimeout(imageUrl, { method: 'GET' }, REQUEST_TIMEOUT_MS);
        if (!imageResponse.ok) {
...
async function generateCardImage(cardData, playerName) {
    try {
        console.log(`  → Generating image...`);
        const response = await fetchWithTimeout(SHOWDOWN_BOT_IMAGE_API, {
            method: 'POST',
            headers: {
...
function transformChart(chartResults) {
    const entries = [];
...
function transformToPlayerCard(response, imageUrl) {
    const { card } = response;
    const playerType = card.player_type === 'PITCHER' || card.player_type === 'Pitcher' ? 'Pitcher' : 'Hitter';
...
async function generateCard(name, year) {
    try {
        console.log(`\n[${name} ${year}]`);
        ...
        const response = await fetchWithTimeout(SHOWDOWN_BOT_API, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function populateDatabase() {
    ...
    for (const { name, year } of POPULAR_PLAYERS) {
        try {
            ...
            await pool.query(`INSERT INTO player_cards (name, year, card_data) 
         VALUES ($1, $2, $3)
         ON CONFLICT (name, year) DO UPDATE SET card_data = $3, updated_at = NOW()`, [card.name, card.year, card]);
